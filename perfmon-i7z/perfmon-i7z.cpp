...
void Read_Thermal_Status(struct therm_status_struct* therm_status) {
    unsigned long val = get_msr_value(0, IA32_THERM_STATUS, 63, 0, &error_indx);
    ...
    therm_status->current_core_temp = therm_status->PROCHOT_temp - therm_status->digital_readout;
}
...
void* main_thread(void* filename) {
    ...
    ******************************************/
    ...
    unsigned long new_value_array[MAX_PROCESSORS_SYSTEM * 4], old_value_array[MAX_PROCESSORS_SYSTEM * 4];

    unsigned long instruction_retired_any_old[MAX_PROCESSORS_SYSTEM], instruction_retired_any_new[MAX_PROCESSORS_SYSTEM];
    unsigned long cpu_clk_unhalted_core_old[MAX_PROCESSORS_SYSTEM], cpu_clk_unhalted_core_new[MAX_PROCESSORS_SYSTEM];
    unsigned long cpu_clk_unhalted_ref_old[MAX_PROCESSORS_SYSTEM], cpu_clk_unhalted_ref_new[MAX_PROCESSORS_SYSTEM];
    unsigned long new_tsc[MAX_PROCESSORS_SYSTEM], old_tsc[MAX_PROCESSORS_SYSTEM];

    memset(new_value_array, 0, MAX_PROCESSORS_SYSTEM * 4 * sizeof(unsigned long));
    memset(old_value_array, 0, MAX_PROCESSORS_SYSTEM * 4 * sizeof(unsigned long));
    memset(instruction_retired_any_old, 0, MAX_PROCESSORS_SYSTEM * sizeof(unsigned long));
    memset(instruction_retired_any_new, 0, MAX_PROCESSORS_SYSTEM * sizeof(unsigned long));
    memset(cpu_clk_unhalted_core_old, 0, MAX_PROCESSORS_SYSTEM * 4 * sizeof(unsigned long));
    memset(cpu_clk_unhalted_core_new, 0, MAX_PROCESSORS_SYSTEM * 4 * sizeof(unsigned long));
    memset(cpu_clk_unhalted_ref_old, 0, MAX_PROCESSORS_SYSTEM * 4 * sizeof(unsigned long));
    memset(cpu_clk_unhalted_ref_new, 0, MAX_PROCESSORS_SYSTEM * 4 * sizeof(unsigned long));
    ...
    unsigned long long tsc_overall, instruction_retired_any_overall, cpu_clk_unhalted_core_overall, cpu_clk_unhalted_ref_overall, value_array_overall[4];
    ...
    for (i = 0; i < MAX_PROCESSORS_SYSTEM; i++) {
        ...
        Set_MV1_Counters(i, FP_COMP_OPS_EXE_SSE_FP, FP_COMP_OPS_EXE_X87, MEM_INST_RETIRED_LOADS, MEM_INST_RETIRED_STORES);
    ...
    while (1) {
        ...
        tsc_overall = 0, instruction_retired_any_overall = 0, cpu_clk_unhalted_core_overall = 0, cpu_clk_unhalted_ref_overall = 0;
        ...
        for (i = 0; i < MAX_PROCESSORS_SYSTEM; i++) {
            ...
            if ((iteration > 0) && (socket_0.processor_num[i] != -1)) {
                num_online_physical_cores++;
                if (iteration % 10 == 0) {
                    printf("%10d\t%12.2Lf\t", socket_0.processor_num[i], ctime_long_double());
                }
                if (logging) {
                ...
                if (iteration % 10 == 0)
                    printf("%10llu\t%10f\t\t", (new_tsc[i] - old_tsc[i]),
                           CPU_FREQUENCY * ((double) (cpu_clk_unhalted_core_new[i] - cpu_clk_unhalted_core_old[i]))
                               / ((double) (cpu_clk_unhalted_ref_new[i] - cpu_clk_unhalted_ref_old[i])));
                if (logging) {
                ...
                if (iteration % 10 == 0) {
                    printf("%10lu\t", instruction_retired_any_new[i] - instruction_retired_any_old[i]);
                }
                if (logging) {
                    fprintf(fp, "%10lu\t", instruction_retired_any_new[i] - instruction_retired_any_old[i]);
                }
                instruction_retired_any_overall += (instruction_retired_any_new[i] - instruction_retired_any_old[i]);

                if (iteration % 10 == 0) {
                    printf("%10lu\t", cpu_clk_unhalted_core_new[i] - cpu_clk_unhalted_core_old[i]);
                }
                if (logging) {
                    fprintf(fp, "%10lu\t", cpu_clk_unhalted_core_new[i] - cpu_clk_unhalted_core_old[i]);
                }
                cpu_clk_unhalted_core_overall += (cpu_clk_unhalted_core_new[i] - cpu_clk_unhalted_core_old[i]);

                if (iteration % 10 == 0) {
                    printf("%10lu\t", cpu_clk_unhalted_ref_new[i] - cpu_clk_unhalted_ref_old[i]);
                }
                if (logging) {
                    fprintf(fp, "%10lu\t", cpu_clk_unhalted_ref_new[i] - cpu_clk_unhalted_ref_old[i]);
                }
                cpu_clk_unhalted_ref_overall += (cpu_clk_unhalted_ref_new[i] - cpu_clk_unhalted_ref_old[i]);

                if (iteration % 10 == 0) {
                    printf("%10lu\t", new_value_array[i * 4 + 0] - old_value_array[i * 4 + 0]);
                }
                if (logging) {
                    fprintf(fp, "%10lu\t", new_value_array[i * 4 + 0] - old_value_array[i * 4 + 0]);
                }
                value_array_overall[0] += (new_value_array[i * 4 + 0] - old_value_array[i * 4 + 0]);

                if (iteration % 10 == 0) {
                    printf("%10lu\t", new_value_array[i * 4 + 1] - old_value_array[i * 4 + 1]);
                }
                if (logging) {
                    fprintf(fp, "%10lu\t", new_value_array[i * 4 + 1] - old_value_array[i * 4 + 1]);
                }
                value_array_overall[1] += (new_value_array[i * 4 + 1] - old_value_array[i * 4 + 1]);

                if (iteration % 10 == 0) {
                    printf("%10lu\t", new_value_array[i * 4 + 2] - old_value_array[i * 4 + 2]);
                }
                if (logging) {
                    fprintf(fp, "%10lu\t", new_value_array[i * 4 + 2] - old_value_array[i * 4 + 2]);
                }
        ...
        if ((iteration > 0) && (iteration % 10 == 0)) {
            printf("Overall (%d/%d)\t%12.2Lf\t", num_online_physical_cores, 4, ctime_long_double());
            printf("%10llu\t%10f\t\t", tsc_overall, CPU_FREQUENCY * ((double) cpu_clk_unhalted_core_overall / (double) cpu_clk_unhalted_ref_overall));
        ...
        if ((logging) && (iteration > 0)) {
            fprintf(fp, "Overall (%d/%d)\t%12.2Lf\t", num_online_physical_cores, 4, ctime_long_double());
            fprintf(fp, "%10llu\t%10f\t", tsc_overall, CPU_FREQUENCY * ((double) cpu_clk_unhalted_core_overall / (double) cpu_clk_unhalted_ref_overall));
            fprintf(fp, "%10llu\t", instruction_retired_any_overall);
