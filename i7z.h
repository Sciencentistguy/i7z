...
void get_CPUs_info(unsigned int* num_Logical_OS, unsigned int* num_Logical_process, unsigned int* num_Processor_Core, unsigned int* num_Physical_Socket);
...
void construct_socket_information(struct cpu_heirarchy_info* chi, struct cpu_socket_info* socket_0, struct cpu_socket_info* socket_1, int, int);
...
#define SET_ONLINE_ARRAY_MINUS1(online_cpus)       \
    {                                              \
        int iii;                                   \
        for (iii = 0; iii < MAX_PROCESSORS; iii++) \
            online_cpus[iii] = -1;                 \
    }
#define SET_ONLINE_ARRAY_PLUS1(online_cpus)        \
    {                                              \
        int iii;                                   \
        for (iii = 0; iii < MAX_PROCESSORS; iii++) \
            online_cpus[iii] = 1;                  \
...
#define SET_IF_TRUE(error_indx, a, b) \
    if (error_indx)                   \
        a = b;
